...
def uses_stack_collate(encoder_type: str) -> bool:
    """Encoders that need conn_stack/adj_bank/full_adj tensors in the batch dict."""
    encoder_l = str(encoder_type).lower()
    return encoder_l in BANK_ENCODERS or encoder_l in MULTIBAND_ENCODERS or encoder_l == "linkx_cnn"
...
class BankCandidateFusion(nn.Module):
    def __init__(self, num_candidates: int, emb_dim: int, mode: str = "concat", hidden_dim: Optional[int] = None, dropout: float = 0.0):
        super().__init__()
        ...
        if self.mode == "concat":
            ...
            raise ValueError("candidate_fusion_mode must be one of: concat, gated, mean")

    def forward(self, view_embs: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        # view_embs: [B,K,D]
        if view_embs.ndim != 3:
            raise ValueError(f"Expected view_embs [B,K,D], got {tuple(view_embs.shape)}")
        if self.mode == "concat":
            return self.proj(view_embs.reshape(view_embs.shape[0], -1)), None
        if self.mode == "mean":
            return view_embs.mean(dim=1), None
        scores = self.gate(view_embs).squeeze(-1)       # [B,K]
        alpha = torch.softmax(scores, dim=1)
        return torch.sum(alpha.unsqueeze(-1) * view_embs, dim=1), alpha
...
class RawNodeBankEdgeMLPEncoder(nn.Module):
    ...
    def forward(self, pyg_batch: Batch, adj_bank: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        bank_emb, alpha = self.candidate_fusion(edge_emb)
...
class FusedBankLinkXEncoderLocal(nn.Module):
    """Fuse K candidate adjacencies first, then run a LinkX-style node+edge encoder."""
    def __init__(
        self,
        *,
        num_nodes: int,
        ...
        self.num_node_features = int(num_node_features)
        self.num_candidates = int(num_candidates)
        self.bank_fusion_mode = str(bank_fusion_mode).lower()
        self.adj_value_mode = str(adj_value_mode).lower()
        self.static_logits = nn.Parameter(torch.zeros(self.num_candidates))
        if self.bank_fusion_mode in {"gated", "summary_gated"}:
            self.summary_gate = nn.Sequential(nn.Linear(4, 32), nn.ReLU(), nn.Linear(32, 1))
        elif self.bank_fusion_mode in {"static", "mean"}:
            self.summary_gate = None
        else:
            raise ValueError("bank_fusion_mode must be one of: static, mean, gated, summary_gated")
        self.linkx = RawNodeBankEdgeMLPEncoder(
            ...
            dropout=dropout,
            candidate_fusion_mode="mean",
            edge_value_mode="raw",
        )

    def _fuse(self, adj_bank: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        adj_bank = _prepare_dense_stack(adj_bank, num_nodes=self.num_nodes, value_mode=self.adj_value_mode)
        B, K, N, _ = adj_bank.shape
        if K != self.num_candidates:
            raise ValueError(f"Expected num_candidates={self.num_candidates}, got K={K}")
        if self.bank_fusion_mode == "mean":
            alpha = torch.ones(B, K, device=adj_bank.device, dtype=adj_bank.dtype) / float(K)
        elif self.bank_fusion_mode == "static":
            alpha = torch.softmax(self.static_logits, dim=0).view(1, K).expand(B, K)
        else:
            vals = torch.stack([
                adj_bank.mean(dim=(-1, -2)),
                adj_bank.std(dim=(-1, -2)),
                adj_bank.abs().mean(dim=(-1, -2)),
                (adj_bank.abs() > 1e-8).float().mean(dim=(-1, -2)),
            ], dim=-1)  # [B,K,4]
            scores = self.summary_gate(vals).squeeze(-1)
            alpha = torch.softmax(scores, dim=1)
        fused = torch.sum(alpha.view(B, K, 1, 1) * adj_bank, dim=1)
        return fused, alpha

    def forward(self, pyg_batch: Batch, adj_bank: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        fused, alpha = self._fuse(adj_bank)
        graph_emb, aux = self.linkx(pyg_batch, fused.unsqueeze(1))
        aux["fusion_weights"] = alpha.detach()
        return graph_emb, aux
    ...
    def __init__(
        ...
        **_: Any,
        ...
        if self.encoder_type == "linkx_bank":
            ...
            self.graph_encoder = FusedBankLinkXEncoderLocal(
                num_nodes=num_nodes,
                num_node_features=num_node_features,
                num_candidates=num_candidates,
                node_hidden_dims=node_hidden_dims,
...
# ---------------------------------------------------------
# Graph-bank construction helpers
# ---------------------------------------------------------
def collect_required_connectivity_metrics(bank_specs: Optional[Sequence[Dict[str, Any]]], default_connectivity_metric: str) -> List[str]:
...
def _payload_with_selected_connectivity_band(payload: Dict[str, Dict[str, Any]], metric: str, band: Optional[Union[int, str]]) -> Dict[str, Dict[str, Any]]:
    """
    Make a shallow payload copy where payload[sid]['connectivity'][metric] is [W,N,N].
    This keeps build_graphs_from_payload compatible with both old and new signatures.
    """
    out: Dict[str, Dict[str, Any]] = {}
    for sid, subj in payload.items():
        new_subj = dict(subj)
        new_subj["features"] = subj["features"]
        new_conn = dict(subj.get("connectivity", {}))
        if metric not in new_conn:
            raise KeyError(f"payload[{sid!r}]['connectivity'] missing metric {metric!r}")
        arr = np.asarray(new_conn[metric], dtype=np.float32)
        if arr.ndim == 4:
            if band is None:
                raise ValueError(
                    f"Connectivity metric {metric!r} for subject {sid!r} is banded [W,B,N,N], "
                    "but the bank candidate did not specify connectivity_band."
                )
            if isinstance(band, str):
                raise ValueError("String band names should be sliced in load_h5_payload_for_subjects; use integer band in bank_specs here.")
            arr = arr[:, int(band)]
        elif arr.ndim != 3:
            raise ValueError(f"Connectivity metric {metric!r} must be [W,N,N] or [W,B,N,N], got {arr.shape}")
        new_conn[metric] = arr
        new_subj["connectivity"] = new_conn
        out[sid] = new_subj
    return out
...
def build_graph_bank_from_specs(
    ...
    *,
    ...
    bank_specs: Optional[Sequence[Dict[str, Any]]],
    standardize_features: bool = True,
) -> Tuple[List[Data], List[str]]:
    if bank_specs is None or len(bank_specs) == 0:
        bank_specs = default_bank_specs_for_metric(default_connectivity_metric, default_filter_method, num_bands=5)
    ...
    for spec_idx, spec in enumerate(bank_specs):
        ...
        cand_metric = str(spec.get("connectivity_metric", default_connectivity_metric))
        cand_band = spec.get("connectivity_band", default_connectivity_band)
        cand_filter_method = str(spec.get("filter_method", default_filter_method))
        cand_fixed_edges = spec.get("fixed_edges", default_fixed_edges)

        sliced_payload = _payload_with_selected_connectivity_band(payload, cand_metric, cand_band)
        gs = _call_with_supported_kwargs(
            build_graphs_from_payload,
            sliced_payload,
            subject_ids,
    ...
    def _graph_key(g: Data) -> Tuple[str, int, int]:
        return (
    ...
    for gs in candidate_graph_lists:
        candidate_maps.append({_graph_key(g): g for g in gs})
    ...
    for g in base_graphs:
        ...
        for cand_name, gmap in zip(candidate_names, candidate_maps):
            ...
            if not hasattr(gg, "adj") or gg.adj is None:
                raise ValueError(f"Candidate {cand_name!r} graph for key {key} is missing dense g.adj.")
            adj = gg.adj.detach().cpu().float() if torch.is_tensor(gg.adj) else torch.tensor(gg.adj, dtype=torch.float32)
            bank_adj.append(adj)
            bank_topo.append((adj.abs() > 1e-8).float())
        g.adj_bank = torch.stack(bank_adj, dim=0).float()          # [K,N,N]
        g.topology_bank = torch.stack(bank_topo, dim=0).float()    # [K,N,N]
        g.topology_names = list(candidate_names)
        g.conn_stack = g.adj_bank                                  # CNN-bank compatibility
...
def build_split_graphs(
    *,
    ...
    if need_build:
        ...
        print(f"[H5] connectivity_metrics={required_connectivity_metrics}")
...
def collate_subject_bags_bank_aware(batch: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    """Subject-bag collate that preserves full_adj, conn_stack, adj_bank, topology_bank."""
    all_graphs: List[Data] = []
    bag_sizes: List[int] = []
    labels: List[int] = []
    subject_ids: List[str] = []
    segment_ids_per_subject: List[List[int]] = []
    ...
    for item in batch:
        ...
        if "segment_ids" in item:
            segment_ids_per_subject.append(list(item["segment_ids"]))

    flat = collate_graph_segments(all_graphs)
    out: Dict[str, Any] = {
        "pyg_batch": flat["pyg_batch"],
        "bag_sizes": torch.tensor(bag_sizes, dtype=torch.long),
        "labels": torch.tensor(labels, dtype=torch.long),
        "subject_ids": subject_ids,
    }
    for k in ["full_adj", "conn_stack", "adj_bank", "topology_bank", "topology_names"]:
        if k in flat:
            out[k] = flat[k]
...
class SegmentGraphClassifierFromMIL(nn.Module):
    ...
    def forward(self, batch_dict: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        elif self.encoder_type in BANK_ENCODERS:
            adj_bank = batch_dict.get("adj_bank", None)
            conn_stack = batch_dict.get("conn_stack", None)
            if adj_bank is None and conn_stack is not None:
                adj_bank = conn_stack
...
def run_caueeg_linkx_training(
    *,
    ...
    if training_approach == "mil":
        ...
        collate_fn = collate_subject_bags_bank_aware
        ...
        model_kwargs = build_model_kwargs(
            ...
            edge_hidden_dims=edge_hidden_dims,
            branch_emb_dim=branch_emb_dim,
            cnn_num_bands=getattr(train_graphs[0], "conn_stack", torch.empty(0)).shape[0] if (multiband_encoder or bank_encoder) else None,
        ...
        model = make_subject_model(**model_kwargs).to(device_t)
    ...
    summary_test.update({
        ...
        "bank_fusion_mode": bank_fusion_mode if encoder_l == "linkx_fused_bank" else None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--training_approach", type=str, default="mil", choices=["mil", "segment_k", "segment_all"])
    ...
    parser.add_argument("--out_h5", type=str, default=None)
    parser.add_argument("--rebuild_h5", action="store_true")
    parser.add_argument("--output_root", type=str, default="/home/anphan/Documents/CAUEEG/result_all_training-paper")
    ...
    parser.add_argument("--bank_specs_json", type=str, default=None, help="Path to JSON list of bank candidate specs, or inline JSON string.")
    parser.add_argument("--candidate_fusion_mode", type=str, default="concat", choices=["concat", "gated", "mean"])
    parser.add_argument("--candidate_fusion_hidden_dim", type=int, default=None)
    parser.add_argument("--candidate_fusion_dropout", type=float, default=0.0)
    parser.add_argument("--bank_fusion_mode", type=str, default="static", choices=["static", "mean", "gated", "summary_gated"])
    parser.add_argument("--graph_backbone", type=str, default="gatv2", choices=["gatv2", "gcn", "sage"])
