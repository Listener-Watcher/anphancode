...
class RawEdgeMLPEncoder(nn.Module):
    """
    ...
    def _get_full_adj(self, full_adj, device, dtype):
        if full_adj is None:
            raise ValueError(
                "edge_mode='full_adj' requires full_adj to be passed into forward()."
            )

        if full_adj.dim() != 3:
            raise ValueError(f"Expected full_adj shape [B, N, N], got {tuple(full_adj.shape)}")
        ...
        return full_adj.to(device=device, dtype=dtype)



    def forward(self, pyg_batch, full_adj=None):
        """
        ...
        elif self.edge_mode == "full_adj":
            adj = self._get_full_adj(
                full_adj=full_adj,
        ...
        graph_emb = self.fusion(edge_emb)                # [num_graphs, emb_dim]
    ...
    def __init__(
        ...
        if self.encoder_type == "mlp_edge":
            ...
            raise ValueError(
                ...
                f"Choose from ['gnn', 'mlp_edge', 'mlp_node', 'sage', 'gcn2', 'h2gcn']"
            )
    ...
    def forward(self, batch_dict: Dict):
        if self.encoder_type == "mlp_edge":
            graph_emb = self.graph_encoder(
                ...
                full_adj=batch_dict.get("full_adj", None),
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature_families", type=str, required=False, default="relative_band_power")   # e.g. "relative_band_power,hjorth"
    ...
    parser.add_argument(
        ...
        type=str,
        default="gnn",
        choices=["gnn", "mlp_edge", "mlp_node", "sage", "gcn2", "h2gcn"],
    )
    parser.add_argument(
        "--edge_mode",
        type=str,
        default="topology_weighted",
        choices=["topology_weighted", "topology_binary", "full_adj"],
    )
    ...
    if args.encoder_type in ["mlp_edge", "mlp_node"]:
        start_epoch=50
        epochs=200
        patience=50
    else:
    ...
    if "mono" in parts:
        ...
        last_part = os.path.basename(os.path.dirname(all_data_path))
        print(last_part)
        parts = last_part.split('_')
        if "mono" in parts:
            channel_names = config.MONO_CHANNELS
    ...
    check_term = f"{args.encoder_type}_{mil_pool_type}_{channel_name}_{topology}_{edge_mode}_{args.connectivity_metric}_{args.connectivity_band}_{args.base_k}_{args.dim}"
    ...
    folder_name = f"{timestamp}_{last_part}_{check_term}"
