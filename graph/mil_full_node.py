...
class RawNodeEdgeMLPEncoder(nn.Module):
    ...
    def forward(self, pyg_batch):
        ...
        # ----- fuse -----
        fused = torch.cat([node_emb], dim=1)
    ...
    def __init__(
        ...
        if self.encoder_type == "sage":
            ...
            if num_nodes is None:
                raise ValueError("num_nodes must be provided when encoder_type='mlp'")
            ...
            raise ValueError(
                ...
                f"Choose from ['gnn', 'mlp', 'sage', 'gcn2', 'h2gcn']"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="gnn",
        choices=["gnn", "mlp", "sage", "gcn2", "h2gcn"],
    )
    ...
    save_path = os.path.join(root_path,'result_Apr01_MLPNode')
