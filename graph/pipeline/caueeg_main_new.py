...
def validate_spec(spec: CAUEEGExperimentSpec) -> None:
    ...
    if spec.model.family == "segment_mvgnn":
        if spec.level.graph_level != "segment":
            raise ValueError("segment_mvgnn requires level.graph_level='segment'.")
        if spec.aggregation.strategy == "none":
            raise ValueError("segment_mvgnn is a bag-level subject model; use a non-'none' aggregation strategy.")
        _require_graph_helpers()
        if spec.topology.strategy != "fused_bank" and not spec.topology.graph_bank_specs:
            raise ValueError("segment_mvgnn expects a graph bank configuration.")
        return
...
def run_caueeg_experiment(spec: CAUEEGExperimentSpec) -> dict[str, Any]:
    ...
    if spec.model.family == "segment_mvgnn":
        from segment_mvgnn_new import run_segment_mvgnn_experiment
        return run_segment_mvgnn_experiment(spec)
...
if __name__ == "__main__":
    ...
    spec = CAUEEGExperimentSpec(
        ...
        #             "edge_pairs": default_fixed_edge_pairs_19(),
        #         },
        #         # {
    ...
    # write the python code into the new file to randomly pick 1 segment from each set (training /validation/ test), then plot all the graph / connectivity matrix in bank, then visualize the fused graph (after the learning step and before the encoding step, and the attention score for each graph), so that we can make sure the model is learning correctly