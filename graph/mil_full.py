...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hyperedge_weight_mode",
        type=str,
        default="mean_abs_adj",
        choices=["mean_abs_adj", "mean_adj", "ones"],
    )
    parser.add_argument(
        "--clique_combine_mode",
        type=str,
    ...
    if topology =="hypergraph":
        save_path = os.path.join(root_path,'result_Apr07-LINKXhypergraph')
    else:
        save_path = os.path.join(root_path,'result_Apr06-mlpnode')
    ...
    check_term = f"{args.encoder_type}_{mil_pool_type}_{channel_name}_{topology}_{feature_name_list}_{args.connectivity_metric}_{args.connectivity_band}_{args.base_k}_{args.dim}"
    ...
    with open(log_path, "w") as f:
        ...
        if topology =="hypergraph":
            f.write(f"hyperedge_weight_mode={args.hyperedge_weight_mode}, clique_combine_mode={args.clique_combine_mode}\n")
    ...
    pred_rows = []
    if topology == "hypergraph":
        graphs = build_graphs_from_master_h5_region_clique(
            h5_path=all_data_path,
            feature_families=feature_families,
            connectivity_metric=args.connectivity_metric,
            connectivity_band=args.connectivity_band,
            channel_names=channel_names,
            region_to_channels=config.region_hyperedges,
            subject_ids=sub_id_list,
            standardize_features=standardize_features,
            hyperedge_weight_mode=args.hyperedge_weight_mode,   # or "mean_adj", "ones"
            clique_combine_mode=args.clique_combine_mode,              # or "max"
        )
    else:
