...
from mil_utils import SubjectBagGraphDataset
...
def select_global_cluster_proportional_random_graphs_from_manifest(
    ...
    *,
    ...
    ensure_min_one_per_cluster: bool = True,
    ...
    """
    Select k training segments per subject using subject-specific global-cluster distribution.

    Cluster allocation:
        cluster_weight = cluster_size ** cluster_weight_alpha

    Recommended:
        cluster_weight_alpha=0.5
        ensure_min_one_per_cluster=True

    This respects dominant clusters but prevents rare clusters from disappearing.
    """
    rng = np.random.default_rng(seed)
    ...
    for sid, sdf in df.groupby("subject_id"):
        ...
        cluster_sizes = sdf[cluster_col].value_counts().sort_index()
        cluster_ids = cluster_sizes.index.to_numpy(dtype=int)
        cluster_counts = cluster_sizes.to_numpy(dtype=np.float64)

        allocation = {int(c): 0 for c in cluster_ids}

        # -------------------------------------------------
        # Step 1: guarantee coverage if possible.
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # Step 2: allocate remaining slots by cluster size.
        # -------------------------------------------------
        if remaining_slots > 0:
            ...
            if uniform_cluster_mix > 0:
                ...
                weights = (
                    (1.0 - float(uniform_cluster_mix)) * weights
                    + float(uniform_cluster_mix) * uniform
                )
                weights = weights / weights.sum()

            for _ in range(remaining_slots):
                # Prefer clusters that still have unused unique segments.
                available = []
                available_weights = []

                for c, base_w in zip(cluster_ids, weights):
                    c = int(c)
                    n_c = int(cluster_sizes.loc[c])
                    if allocation[c] < n_c:
                        available.append(c)
                        available_weights.append(base_w)

                # If no unique capacity remains, allow replacement if requested.
                if len(available) == 0:
                    if not fill_with_replacement:
                        break
                    available = [int(c) for c in cluster_ids]
        ...
        # -------------------------------------------------
        # Step 3: sample segments inside each allocated cluster.
        # -------------------------------------------------
        chosen_rows = []
        ...
        # Safety: if somehow fewer than k, fill from subject pool.
        if len(chosen_df) < k:
            need = k - len(chosen_df)
            replace = bool(fill_with_replacement and need > len(sdf))
        ...
        for _, row in chosen_df.iterrows():
            ...
            # copy.copy helps when replacement duplicates the same graph.
            g = copy.copy(graph_lookup[key])
            selected_graphs.append(g)
...
def run_caueeg_linkx_mil(
    ...
    if encoder_type in ['linkx_cnn','linkx_cnn5', "cnn5", 'LINKX', 'mlp_node', 'linkx_bank', 'linkx_fused_bank',]:
        patience=50
        start_epoch=50
        epochs=200
        lr=1e-3
    ...
    if selection_strategy != "original_random_k":
        ...
        if selection_strategy.startswith("global_cluster"):
    ...
    if selection_strategy == "original_random_k":
        ...
        # Fixed KMeans-representative clean segments.
    ...
    from mil_utils import plot_mil_learning_curves, plot_mil_learning_summary
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(root_path,'result_MIL-LinkX-kmeans')
    os.makedirs(save_path,exist_ok = True)



    parser = argparse.ArgumentParser(description="Run script with directory path and model name")
    parser.add_argument("--out_h5", type=str, required=True, help="out_h5")
    ...
    parser.add_argument(
        ...
        type=str,
        default="LINKX",
        choices=["gnn", "gnn_bank","cnn_bank", "linkx_cnn_bank", 'linkx_bank', 'linkx_fused_bank', 'hybrid', 'gat', "LINKX", "linkx_cnn", "cnn5","linkx_cnn5", "mlp_node", "sage", "gcn2", "h2gcn"]
    )
    ...
    run_name = (
        ...
        f"{args.connectivity_metric}_{k_tag}_{norm_tag}_{args.encoder_type}_{args.mil_pool_type}_{args.topology}"
    )
    
    if segment_selection_strategy.startswith("global_cluster"):
