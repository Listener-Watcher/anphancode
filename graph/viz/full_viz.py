...
if __name__ == "__main__":
    ...
    for file_path, label in zip(data_paths, labels):
        ...
        sid = parts[0]
        # print(sid)
        # if sid in ['sub-009', 'sub-010', 'sub-016', 'sub-033', 'sub-044', 'sub-051', 'sub-061', 'sub-077', 'sub-087']:
        #     continue

        # eeg_data = preprocess_eeg_update(file_path, dataset, sid, subject_folder)  # (n_channels, timepoints)
        ...
        elif electrode in ['bi23','bi30']:
            bipolar_data, bipolar_names = convert_to_bipolar(
                eeg_data, mono_channel, channel_names
            )
            print("bipolar_names", bipolar_names)
        ...
        for seg_idx, start in enumerate(range(0, num_windows * step_size, step_size)):

            if electrode in ['bi23','bi30']:
                eeg_window = bipolar_data[:, start:start + window_size]
            elif electrode == 'mono':
    ...
    # save_summary_statistics(all_data, filename=f"{data_processed_path}/analysis_summaries.pt")
    # print("Saved all raw adjacency matrices and node features.")
    # title = f"{feature_name}_{edge_methods}_{band_name}"
    # plot_connectivity_heatmap(all_data, channel_names, class_names, folder_path, title)
    # plot_class_contrast(all_data, channel_names, class_folder)

    # # plot_class_contrast(all_data, class_folder, class_a=0, class_b=1)
    # # plot_class_contrast(all_data, class_folder, class_a=1, class_b=2)
    # # plot_class_contrast(all_data, class_folder, class_a=0, class_b=2)
    ...
    # # unique_subjects = {item['subject_id'] for item in all_data}
    # # num_unique = len(unique_subjects)

    # # if num_unique != 88:
    # #     print(f"ERROR: Expected 88 subjects, but found {num_unique}.")
    # #     print("Stopping script execution.")
    # #     sys.exit(1)

    # df = create_feature_dataframe(all_data, formatted_channels, actual_feature_names)
    ...
    # level = 'segment'
    # # 1. Prepare your data
    # adj_matrices = np.stack([d['adj'].numpy() for d in all_data])
    # print(adj_matrices.shape)

    # labels = np.array([d['class_id'] for d in all_data])
    # # analyze_connectivity_with_fdr(adj_matrices, labels, channel_names)
    # # analyze_connectivity_stats(adj_matrices, labels, channel_names)
    # # analyze_connectivity_anova(adj_matrices, labels)

    # # 2. Run the function
    # df_all, df_sig = analyze_connectivity_with_fdr(adj_matrices, labels, channel_names)
    # df_all.to_csv(f"{data_processed_path}/connectivity_full_stats_fdr_Segment.csv", index=False)
    # df_sig.to_csv(f"{data_processed_path}/connectivity_significant_only_Segment.csv", index=False)
    # save_top_edge_comparison(df_sig, adj_matrices, labels, channel_names, data_processed_path, level)
    # # plot_significant_chord(df_sig, channel_names, class_folder)
    # # plot_top_significant_chord(df_sig, channel_names,class_folder, top_n=20)
    ...
    # # Save to CSV
    # df_sorted.to_csv(f"{data_processed_path}/significant_edges.csv", index=False)
    # with open(log_path, "a") as f:
    ...
    # df_all.to_csv(f"{data_processed_path}/connectivity_full_stats_fdr_Subject.csv", index=False)
    # df_sig.to_csv(f"{data_processed_path}/connectivity_significant_only_Subject.csv", index=False)
    # save_top_edge_comparison(df_sig, adj_matrices_subjects, labels_subjects, channel_names, data_processed_path, level)
...
# def plot_subject_channel_boxplot(data_list, target_sid, feature_idx, feature_name, channel_names, save_folder):
#     """
#     Plots a boxplot for ONE subject: X-axis = Channels, Y-axis = Value.
#     If you have multiple segments for this subject, the boxplot shows the distribution.
#     """
#     # 1. Filter segments for this specific subject
#     subj_segments = [d for d in data_list if d['subject_id'] == target_sid]
    
#     rows = []
#     for d in subj_segments:
#         # node_features shape: (n_channels, m_features)
#         # Extract the specific feature column
#         feat_column = d['node_features'][:, feature_idx].numpy()
        
#         for ch_idx, val in enumerate(feat_column):
#             rows.append({
#                 'Channel': channel_names[ch_idx],
#                 'Value': val,
#                 'Class': f"Class {d['class_id']}"
#             })
    
#     df = pd.DataFrame(rows)

#     plt.figure(figsize=(15, 6))
#     sns.boxplot(x='Channel', y='Value', hue='Class', data=df)
#     plt.xticks(rotation=45)
#     plt.title(f"Subject {target_sid}: Channel-wise {feature_name} Distribution")
#     plt.tight_layout()
#     plt.savefig(f"{save_folder}/{s_id}_nodefeature_boxplot.png")
#     plt.close()
#     # plt.show()

# def plot_class_feature_heatmaps(data_list, feature_names, channel_names, save_folder):
#     """
#     Plots 3 heatmaps (one for each class) showing Channels vs Features.
#     """
#     unique_classes = sorted(list(set(d['class_id'] for d in data_list)))
#     fig, axes = plt.subplots(1, 3, figsize=(20, 8), sharey=True)

#     for i, c_id in enumerate(unique_classes):
#         # 1. Collect all node_feature matrices for this class
#         class_mats = [d['node_features'] for d in data_list if d['class_id'] == c_id]
        
#         # 2. Average them (Stack then Mean)
#         # Resulting shape: (n_channels, m_features)
#         avg_feat_matrix = torch.stack(class_mats).mean(dim=0).numpy()

#         # 3. Plot on the corresponding subplot
#         sns.heatmap(avg_feat_matrix, ax=axes[i], cmap='viridis', 
#                     xticklabels=feature_names, yticklabels=channel_names)
#         axes[i].set_title(f"Class {c_id} Average")
#         axes[i].set_xlabel("Features")
    
#     axes[0].set_ylabel("Channels")
#     plt.tight_layout()
#     plt.savefig(f"{save_folder}/nodefeature_heatmap.png")
#     plt.close()


    # for sid in df['subject_id'].unique():
    #     subj_path = os.path.join(subject_folder, str(sid))
    #     os.makedirs(subj_path, exist_ok=True)
        
    #     # 1. Heatmap (All channels x All expanded features)
    #     subj_df = df[df['subject_id'] == sid]
    #     # Average across segments to get one value per channel/feature
    #     heatmap_data = subj_df.groupby('channel')[actual_feature_names].mean()
        
    #     plt.figure(figsize=(12, 10))
    #     sns.heatmap(heatmap_data, cmap='viridis', xticklabels=True, yticklabels=True)
    #     plt.title(f"Subject {sid}: All Features")
    #     plt.savefig(f"{subj_path}/feature_fingerprint.png")
    #     plt.close()
        
    #     # 2. Individual Boxplots (e.g., just rbp_alpha)
    #     for feat in actual_feature_names:
    #         plt.figure(figsize=(10, 5))
    #         sns.boxplot(data=subj_df, x='channel', y=feat, hue='class_id')
    #         plt.title(f"Subject {sid}: {feat} across Channels")
    #         plt.savefig(f"{subj_path}/boxplot_{feat}.png")
    #         plt.close()
    # # plot_connectivity_heatmap(all_data, channel_names, class_names, folder_path, title, level="subject")
    # # plot_connectivity_heatmap(all_data, channel_names, class_names, folder_path, title, level="class")
