...
- file       : path to EEG file (.set, .edf, .fif, .npy, .npz)
...
LABEL_TO_INT: Dict[str, int] = {"A": 1, "F": 2, "C": 0}
...
def _canonical_ch_name(name: str) -> str:
    ...
    aliases = {
        ...
        "F4": "F4", "F8": "F8", "T3": "T3", "T7": "T3", "C3": "C3",
        "CZ": "CZ", "C4": "C4", "T4": "T4", "T8": "T4", "T5": "T5",
        "P7": "T5", "P3": "P3", "PZ": "PZ", "P4": "P4", "T6": "T6",
        "P8": "T6", "O1": "O1", "O2": "O2", "A1": "A1", "A2": "A2",
        "M1": "A1", "M2": "A2",
    }
...
def choose_channels(data: np.ndarray, ch_names: Sequence[str], target_channels: Sequence[str]) -> np.ndarray:
    ...
    for i, ch in enumerate(ch_names):
        cname = _canonical_ch_name(str(ch))
        if cname not in canonical_to_idx:
            canonical_to_idx[cname] = i

    target_canonical = [_canonical_ch_name(ch) for ch in target_channels]
    missing = [target_channels[i] for i, ch in enumerate(target_canonical) if ch not in canonical_to_idx]
    if missing:
        raise ValueError(
            f"Missing required EEG channels after alias matching: {missing}. "
            f"Available channels: {list(ch_names)}"
        )
    indices = [canonical_to_idx[ch] for ch in target_canonical]
    return data[indices]
...
def load_subject_eeg(file_path: str, manifest_sfreq: Optional[float], target_channels: Sequence[str]) -> Tuple[np.ndarray, float, List[str]]:
    ...
    if suffix == ".set":
        if mne is None:
            raise ImportError("mne is required to read EEGLAB .set files. Install with `pip install mne`.")
